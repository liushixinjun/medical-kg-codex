...
## 1.1 2026-07-16 开发落地最小闭环

后端和前端必须按“患者数据 → 阶段 → 规则 → 推荐 → 证据”链路使用图谱，不能只查疾病邻居节点。

| 步骤 | 输入 | 图谱查询目标 | 输出给业务 |
|---|---|---|---|
| 1 | 主诉、现病史、诊断、检查检验 | 匹配 Disease、Symptom、Sign、RiskFactor、Exam、LabTest | 疑似疾病和待补检查 |
| 2 | 当前疾病编码 | 查询 DiagnosisCriteria、DiagnosisCriteriaComponent | 诊断依据明细 |
| 3 | 当前疾病编码 | 查询 DifferentialDiagnosis 及鉴别规则 | 需要排除的疾病和排除依据 |
| 4 | 当前疾病 + 患者状态 | 匹配 PathwayStage、ClinicalRule | 当前诊疗阶段 |
| 5 | 当前阶段 + 命中规则 | 查询 RecommendationStatement | 推荐动作、禁忌/排除、优先级 |
| 6 | 推荐编码 | 查询该推荐自己的 Evidence/Guideline | 指南名称、页码、推荐等级、证据等级、原文摘要 |

关键原则：图谱负责“医学事实和证据”，规则/路径引擎负责“患者当前是否满足条件”。前端只展示规则命中后的正式推荐，不把阶段候选动作当正式推荐。
