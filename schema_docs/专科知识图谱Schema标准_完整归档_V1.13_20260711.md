...
变更说明：在知识图谱标准基础上补充专病 CDSS 动态流程引擎建模边界，明确诊断标准必须拆解为可推理组件，将 `RecommendationStatement` 作为 CDSS 推荐证据根实体，并收编教材骨架拆解实体、来源章节实体、字段层与关系层区分说明，支持“基础权威教材搭骨架，指南/共识填决策血肉”。
...
本 Schema 支持专病 CDSS 从“知识展示”升级为“患者状态驱动的动态诊疗流程”。该层不替代医院 EMR、Oracle 或前端流程引擎，而是为流程引擎提供可查询、可追溯、可审核的医学规则来源。

职责边界：

| 内容 | 图谱维护 | 流程引擎维护 |
|---|---|---|
| 疾病、症状、体征、检查、药物、操作、诊断标准、鉴别诊断 | 是 | 调用 |
| 诊疗阶段的医学定义、阶段目标、阶段顺序 | 是 | 读取并计算当前患者处于哪一阶段 |
| 指南推荐、适应证、禁忌证、时间窗、证据等级 | 是 | 执行判断并展示 |
| EMR 字段、系统按钮、医嘱提交、检验回报等信息系统事件 | 否 | 是 |
| 当前患者是否满足条件、是否缺资料、是否触发提醒 | 否 | 是 |
| 医生采纳、忽略、暂不处理、批量签收结果 | 否 | 是 |

图谱回答“医学上为什么推荐/禁忌”；流程引擎回答“当前这个患者此刻是否触发”。任何前端或业务系统不得仅按疾病一次性查询全部关联节点作为 CDSS 推荐结果。
...
| `Symptom` | 患者主观症状 |
| `Sign` | 客观体征 |
| `Definition` | 定义容器，保存某来源/章节下的疾病定义原文或定义组 |
| `DefinitionComponent` | 定义拆解组件，用于把定义拆成疾病性质、核心机制、诊断前提、临床意义等可追溯片段 |
| `Etiology` | 病因 |
...
| `Complication` | 并发症 |
| `Prognosis` | 预后 |
| `Prevention` | 预防、二级预防、健康管理和生活方式管理 |
...
| `DiagnosisCriteriaComponent` | 诊断标准明细组件，如症状条件、检查表现、指标阈值、排除条件、动态变化 |
...
| `DiseaseClassification` | **[V1.12 新增]** 疾病分型/分类方案或分类项；新批次优先使用本实体表达疾病分类 |
| `PathwayStage` | **[V1.8 新增]** 专病诊疗路径阶段，如疑诊评估、诊断确认、危险分层、治疗决策、并发症监测、出院随访 |
...
>
> **[V1.12 兼容说明]** 实际骨架库已使用 `DiseaseClassification` 表达疾病分类。新批次优先使用 `DiseaseClassification`；历史 `ClassificationStage` 保留兼容，不再强制迁移。若表达“路径阶段”，必须使用 `PathwayStage`，不得与疾病分型混用。
...
| `Evidence` | 可定位的原文证据分段 |
| `RecommendationStatement` | 正式推荐陈述及分级；CDSS 推荐展示、审核和证据追溯的根实体 |
| `SourceSection` | **[V1.12 新增]** 来源章节或教材/指南章节锚点，用于保存章、节、小标题、页码范围和章节路径 |

> **[V1.10 新增说明 — RecommendationStatement 推荐陈述]**
> `RecommendationStatement` 表示“某指南/共识/教材证据在某临床场景下推荐、暂不推荐或阻断某动作”的独立语义单元。它不是补丁节点，而是推荐本体。  
> `Procedure/Medication/TreatmentPlan` 表示“动作是什么”，不能承载唯一推荐依据；同一动作可被多个疾病、阶段、规则和指南复用。  
> CDSS 前端默认应查询 `RecommendationStatement`，不得从疾病证据池或动作节点证据池二次推理当前推荐依据。
