...
这张图按泳道展示：左侧是医生和 EMR 输入，中间是辅助诊疗规则动作，右侧是图谱真实数据命中。图中使用的是当前图谱导出的 AMI/STEMI 数据锚点，不是临时编造的示例。比如“一诉五史、体征”中的“胸痛2小时、出汗、恶心呕吐、高血压、糖尿病”会先被分拣为具体实体 code，再通过疑似疾病识别规则反查疑似 ACS/AMI，通过初筛检查和鉴别诊断规则推荐检查与鉴别方向，报告回填后通过分型辅助诊断规则收敛分型，最后完成流程准入、专项评估、治疗建议、介入/手术围术期提醒和质控提醒。
