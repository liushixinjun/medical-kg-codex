...
| 时间 | 版本 | 类型 | 说明 |
|---|---|---|---|
| 2026-07-17 17:10:00 | V1.17 | 推荐来源裁决层 | 新增“推荐来源裁决”实体、核心关系、字段要求和硬闸门；正式 CDSS 推荐必须说明主依据指南、裁决理由和冲突状态，以适应未来诊疗指南升级。 |
| 2026-07-15 14:34:29 | V1.16 | 质量闸门增强 | 不新增实体类型；补强“诊断标准必须下钻明细”“临床规则必须符合疾病章节主题归属”“服务器 postcheck 通过才算已入库并通过”。 |
...
7. 临床规则必须通过“疾病范围 + 原文章节主题 + 规则主语义”三重归属，不得只因段落出现疾病名就归入该疾病。
8. 本地审计通过不等于最终完成；写入 Neo4j 后必须以服务器 postcheck 阻断项为 0 作为完成标准。
9. 正式 CDSS 推荐必须经过推荐来源裁决；没有主依据指南、裁决理由和冲突状态的推荐，只能知识展示，不能主动推荐给医生。
...
| 推荐来源裁决     | `SourceAdjudication`      | 多份指南支持同一推荐时，记录主依据、支持依据、冲突状态和最终 CDSS 使用状态。 | STEMI再灌注治疗来源裁决 |
...
1. `Disease -> has_diagnostic_criteria -> DiagnosisCriteria` 之后，`DiagnosisCriteria` 必须至少有 1 条 `has_diagnostic_component` 明细。
2. 如果原文暂时不能支撑诊断明细，只能标记为待补或 blocked，不能把标题节点标记为 `clinical_ready`。
3. 诊断标准明细必须来自原文证据或权威来源，不得为了通过闸门伪造条件。
...
`ClinicalRule` 是给专病流程和 CDSS 推荐引擎使用的规则，归属必须比普通知识节点更严格。

一条临床规则只有同时满足以下 3 条，才能归入某个疾病：

| 判定项 | 业务含义 | AMI示例 |
|---|---|---|
| 疾病范围命中 | 规则确实服务于该疾病或该疾病分型。 | STEMI 再灌注决策规则属于急性心肌梗死/STEMI。 |
| 原文章节主题一致 | 规则来自该疾病章节、该疾病指南段落或明确的该病推荐小节。 | “STEMI 再灌注治疗”章节中的 PCI/溶栓规则。 |
| 规则主语义一致 | 规则描述的是该疾病的诊断、治疗、随访、禁忌或风险处理。 | 发病 12 小时内、PCI 可及、无溶栓禁忌时推荐再灌注策略。 |

不允许的情况：只因为相邻章节或背景段落提到某个疾病名，就把该段规则归入该疾病。此类内容最多作为来源线索、鉴别诊断、并发症或证据背景，不得进入正式 `ClinicalRule`。
...
推荐来源裁决只用于“正式 CDSS 推荐”，不用于普通知识浏览。它解决的是：多份指南同时存在时，系统最终采用哪份作为主依据，以及未来指南升级后如何平滑替换主依据。

| 起点 | 业务关系 | 关系类型 relationType | 终点 | 格式或使用要求 | AMI示例 |
|---|---|---|---|---|---|
| 疾病 | 有推荐来源裁决 | `has_source_adjudication` | 推荐来源裁决 | 每个正式 CDSS 推荐场景至少一条；按临床问题建，不按整病种泛化。 | 急性心肌梗死 -> STEMI再灌注治疗来源裁决 |
| 推荐来源裁决 | 使用主依据指南 | `uses_primary_guideline` | 指南/教材/共识 | 只能有一个当前主依据；优先专病、当前有效、本土适用、推荐等级高的来源。 | 来源裁决 -> STEMI CN 2019 |
| 推荐来源裁决 | 形成正式推荐 | `decides_recommendation` | 推荐陈述 | 裁决结果落到推荐卡片根实体。 | 来源裁决 -> 推荐急诊PCI |
| 推荐来源裁决 | 推荐/阻断动作 | `recommends_action` / `blocks_action` | 动作节点 | 只能指向标准化动作；动作主名用中文全称，缩写进别名。 | 来源裁决 -> 急诊PCI |
| 推荐来源裁决 | 来自证据 | `derived_from` | 证据片段 | 必须连接主依据原文证据；医生端默认展示这条主证据。 | 来源裁决 -> PCI推荐原文片段 |

支持指南和冲突指南在 V1.17 先作为 `SourceAdjudication` 字段保存；当前端、审计和查询稳定后，再按需要关系化。
...
`SourceAdjudication / 推荐来源裁决` 必须让维护人员看得懂、审计人员查得到、医生端能解释。字段以中文业务含义为准，英文编码只给程序使用。

| 字段 | 中文名 | 格式要求 | AMI示例 |
|---|---|---|---|
| `code` | 裁决编码 | 全局唯一；建议 `SRCADJ-学科-疾病-临床问题-序号`。 | `SRCADJ-CARD-AMI-REPERFUSION-001` |
| `name` | 裁决名称 | 医生可读中文名称。 | STEMI再灌注治疗来源裁决 |
| `disease_code` | 疾病编码 | 必须指向真实 `Disease`。 | `DIS-CARD-CAD-AMI` |
| `clinical_question` | 临床问题 | 按具体问题建，不按整病种泛化。 | STEMI是否需要再灌注治疗 |
| `clinical_scenario` | 适用场景 | 患者条件、阶段、分型或风险状态。 | 发病12小时内、ST段抬高 |
| `final_recommendation` | 最终推荐 | 面向医生的简明推荐语。 | 优先直接PCI；PCI延误时考虑溶栓 |
| `primary_guideline_code` | 主依据指南 | 当前执行优先参考的指南编码。 | `GL-CARD-STEMI-CN-2019` |
| `supporting_guideline_codes` | 支持指南 | 数组；其他一致或补充来源。 | `["GL-CARD-ACS-2025","GL-CARD-INTERNALMED-10"]` |
| `conflict_guideline_codes` | 冲突指南 | 数组；没有冲突用空数组。 | `[]` |
| `adjudication_score` | 裁决得分 | 0-100；来自权重表，不允许大模型黑箱直接给结论。 | `90` |
| `adjudication_reason` | 裁决理由 | 写清楚专病、本土、最新、推荐等级、证据等级等因素。 | 专病指南、本土适用、推荐等级高 |
| `conflict_status` | 冲突状态 | 固定值：无冲突、有冲突已处理、冲突待裁决。 | 无冲突 |
| `cdss_use_status` | CDSS使用状态 | 固定值：正式推荐、仅知识展示、冲突待裁决、证据不足、已过时保留。 | 正式推荐 |
| `last_reviewed_at` | 最近裁决时间 | `YYYY-MM-DD HH:mm:ss`。 | `2026-07-17 17:10:00` |
...
11. `ClinicalRule` 必须通过疾病范围、原文章节主题和规则主语义三重归属校验；关键词命中但章节主题不一致时不得入库为正式规则。
12. 正式 CDSS 推荐必须存在 `SourceAdjudication`，并具备主依据指南、主证据、裁决理由和 CDSS 使用状态。
13. `conflict_status=冲突待裁决` 或 `cdss_use_status` 不是“正式推荐”的内容，不得进入医生端主动推荐区。
...
5. 每次写库后必须按 `batch_id` 运行服务器 postcheck；只有阻断项为 0，批次台账才能标记为“已入库并通过”。否则必须标记为“已入库待收口”或 blocked，并继续修复后复测。
...
患者数据/EMR
  ...
  -> SourceAdjudication 推荐来源裁决
  -> 主依据 Guideline + 主证据 Evidence
```

医生界面只展示当前推荐自己的主依据指南和主证据，不展示疾病级证据池。支持依据和冲突依据放在展开层，供质控和专家复核使用。
