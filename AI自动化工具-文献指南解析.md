...
| 版本 | 变更时间 | 变更内容 |
|---|---|---|
| V1.47 | 2026-07-11 17:34:47 | 补充 Schema V1.15 表格统一格式要求：主 Schema 所有表格最后一列必须提供示例，示例统一优先使用 AMI 场景；涉及具体值的字段必须说明格式要求、正确示例和禁止写法。 |
| V1.47 | 2026-07-11 17:18:30 | 补充 Schema V1.15 §5 字段格式要求：`code`、`id`、`aliases`、状态、版本、时间、证据文本字段必须按统一格式生成；抽取脚本不得再输出自由格式状态或字符串别名。 |
| V1.47 | 2026-07-11 17:06:21 | 同步 Schema V1.15：主 Schema 改为中文优先、英文技术名辅助；补充诊断标准明细、Evidence 证据片段、阶段候选动作与正式推荐动作的中文解释和临床案例；新批次抽取说明必须避免只输出英文关系名。 |
| V1.0 | — | 初版 |
